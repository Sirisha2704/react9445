...
import { DessertData } from '../Data/DessertData'; // Adjust the import if needed
...
function DessertDet() {
  ...
  const dessert = DessertData.find((item) => item.id === parseInt(id)); // Find the dessert by id

  const [cartMessage, setCartMessage] = useState(""); // State to display cart message

  if (!dessert) {
    return <div className="text-center mt-5">Dessert not found!</div>; // Handle case when dessert is not found
  }

  // Function to handle adding the dessert to the cart
  const addToCart = () => {
    let cart = JSON.parse(localStorage.getItem('cart')) || [];
    cart.push(dessert); // Add dessert to cart
    localStorage.setItem('cart', JSON.stringify(cart)); // Save cart to localStorage

    setCartMessage(`${dessert.name} has been added to your cart!`);
    ...
    >
      ...
      <Navbar bg="dark" variant="dark" sticky="top" className="d-flex justify-content-between">
        <Navbar.Brand href="/" className="mx-auto">
          <h2 style={{ color: 'white', margin: '0' }}>Desserts</h2>
        </Navbar.Brand>
        <Nav>
          <Link to="/cart">
